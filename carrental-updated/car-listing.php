...
<title>PakDrive365 - Car Rental | Car Listing</title>
<!--Bootstrap -->
...
<?php $sql = "SELECT tblvehicles.*,tblbrands.BrandName,tblbrands.id as bid,tblcategories.CategoryName  from tblvehicles left join tblbrands on tblbrands.id=tblvehicles.VehiclesBrand left join tblcategories on tblcategories.id=tblvehicles.CategoryId";
$query = $dbh -> prepare($sql);
$query->execute();
$results=$query->fetchAll(PDO::FETCH_OBJ);
$cnt=1;
